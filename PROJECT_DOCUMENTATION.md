...
*   **Databases**: H2 In-Memory Database (configured per service)
...
The project uses independent H2 databases for each service. 
...
*   **Databases**: Uses H2 in-memory `jdbc:h2:mem:<dbname>`. `ddl-auto: update` is used to auto-generate schemas. H2 console is enabled.
...
The project relies on Zookeeper and Kafka. A `docker-compose.yml` is provided at the root:
```bash
docker-compose up -d
```
...
*   **Test Coverage**: Limited to application context startup. Not found in current codebase: extensive unit or integration test suites for business logic.
...
*   **Database**: H2 is currently configured to drop and recreate schemas on restart (`ddl-auto: update`). Data is lost between runs. For persistent data during dev, change the JDBC URL to a file-based path instead of `:mem:`.
*   **Logging**: Avoid using `System.out.println` (currently used in Kafka consumers/producers). Migrate to SLF4J/Logback for proper logging levels.
...
*   Use the H2 console to verify database states. It is accessible at `/h2-console` on the respective service ports (e.g., `http://localhost:8081/h2-console` for User Service) using JDBC URL `jdbc:h2:mem:testdb` (check `application.yml` for specific DB names).
